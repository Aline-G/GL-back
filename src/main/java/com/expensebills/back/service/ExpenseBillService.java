...
public class ExpenseBillService {
    ...
    public HttpStatus addAdvanceToCurrentBill(Advance advance) {
        ...
        if (!billExists) {
            ...
            this.expenseBillRepository.save(
                    ExpenseBill.builder().amount(0).listLineBill(new ArrayList<>()).listAdvance(listAdvance)
                               .date(expenseBillDate).state(BillStates.DRAFT).build());
        }
    ...
    public void checkState(int idExpenseBill) throws ExpenseBillException {
        ...
        if (expense.getState() == BillStates.VALIDATED) {
            ...
            throw new ExpenseBillException("This expense bill is waiting for validation and so can't be modified",
                                           HttpStatus.CONFLICT);
        }
    ...
    public List<ExpenseBill> getExpenseBillList() {
        return StreamSupport.stream(this.expenseBillRepository.findAll().spliterator(), false)
    ...
    public int getNumberBillsNonValidated(int userId) {
        ...
        for (ExpenseBill e : list) {
            if (e.getUser().getId() == userId &&
                (e.getState() == BillStates.DRAFT || e.getState() == BillStates.WAITING ||
                 e.getState() == BillStates.REFUSED)) {
                res += 1;
            }
        }
        return res;
    ...
    public float getTotal(int userId) {
        ...
        List<ExpenseBill> list = getExpenseBillList();
        for (ExpenseBill e : list) {
            if (e.getUser().getId() == userId && (e.getState() == BillStates.DRAFT || e.getState() == BillStates.WAITING ||
                                            e.getState() == BillStates.REFUSED)) {
                total += e.getAmount();
            }
        }
